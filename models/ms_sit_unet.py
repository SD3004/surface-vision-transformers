...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,hidden_dim, num_heads, window_size=80, shift_size=0,
...
class MSSiTUNet(nn.Module):
    ...
    def proj_sphere(self,x):
        ...
        x = self.final_norm(x)
        x = self.output(x) # B, L, num_classes
        x = self.repatch2(x)
        x = rearrange(x, 'b v c -> b c v') # B, num class, Vertices
