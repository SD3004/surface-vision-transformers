...
class MSSiT_shifted(nn.Module):
    ...
    def forward(self,x,confounds=None):
        # input: B,C,L,V
        x = self.forward_features(x,confounds) #B,int(embed_dim * 2 ** i_layer)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,hidden_dim, input_resolution, depth, num_heads,window_size, window_size_factor,
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,hidden_dim, input_resolution, num_heads, window_size=80,window_size_factor=1, shift=0,
        ...
        '''
        if self.shift:
            self.R = torch.Tensor([[ 0.9484011 , -0.22552829, -0.22287293],
                                    [ 0.30020292,  0.86491236,  0.40224971],
                                    [ 0.10204686, -0.44840117,  0.88798808]])
            
            self.coordinates = torch.Tensor((nb.load('./coordinates_ico_6_L.shape.gii').agg_data()).T)
            
            self.rotated_coordinates = torch.matmul(self.coordinates,self.R)

            
            self.R_1 = torch.Tensor([[ 0.9484011 ,  0.30020292,  0.10204686],
                                    [-0.22552829,  0.86491236, -0.44840117],
                                    [-0.22287293,  0.40224971,  0.88798808]])
            
            self.rotated_coordinates_1 = torch.matmul(self.coordinates,self.R_1)
        '''
