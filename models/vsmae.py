...
class vsMAE(nn.Module):
    ...
    def random_masking(self, x, mask_ratio, masking_type):
        ...
        if masking_type == 'tubelet':
            ...
            #x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, V))
            #import pdb;pdb.set_trace()
            x_masked_unshaped = torch.gather(x_unshaped, dim=2, index=ids_keep.unsqueeze(-1).unsqueeze(1).repeat(1, self.nbr_frames ,1, V))
            #import pdb;pdb.set_trace()
            x_masked = rearrange(x_masked_unshaped, 'b t l v -> b (t l) v')
            #import pdb;pdb.set_trace()
            ...
            mask = repeat(mask, 'b l -> b t l', t=self.nbr_frames)
    ...
    def forward_encoder(self, img, confounds=None):
        ...
        x, mask, ids_restore, ids_keep, ids_not_keep = self.random_masking(x, self.masking_ratio,self.masking_type)
        #import pdb;pdb.set_trace()
        # append cls token
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        mask_ext = mask.unsqueeze(-1).repeat(1,1,target.shape[2])
        #import pdb;pdb.set_trace()

        loss = loss[mask_ext==1].mean() # mean loss on removed patches
        #import pdb;pdb.set_trace()
        return loss
