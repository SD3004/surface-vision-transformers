        ...
        if dataset == 'UKB':
            if modality == 'cortical_metrics':
                if task == 'segmentation_msmall':
                    data_path = os.path.join(config['data']['path_to_data'],dataset,'merged_resample_msmall/')
                    labels_path = os.path.join(config['data']['path_to_data'],dataset,'resample_segmentation_maps')
                elif task == 'segmentation':
                    if config['data']['masking_preprocess']:
                        data_path = os.path.join(config['data']['path_to_data'],dataset,'merged_resample_{}_mask/'.format(config['data']['masking_preprocess']))
                        labels_path = os.path.join(config['data']['path_to_data'],dataset,'resample_segmentation_maps_{}_mask'.format(config['data']['masking_preprocess']))  
                    else:
                        data_path = os.path.join(config['data']['path_to_data'],dataset,'merged_resample/')
                        labels_path = os.path.join(config['data']['path_to_data'],dataset,'resample_segmentation_maps')  
...
def save_label_UKB(data, filename):
    label =nb.load('/nfs/home/sdahan/workspace/data/UKB/resample_segmentation_maps/1033131.L.aparc.ico6_fs_LR.label.gii')
    label.darrays[0].data = data
    nb.save(label,filename)

def save_label_MindBoggle(data, filename):
    label =nb.load('/nfs/home/sdahan/workspace/data/MindBoggle/mindboggle_resample_labels_ico6/lh.labels.HLN-12-5.ico6.DKT31.manual.label.gii')
    label.darrays[0].data = data
    nb.save(label,filename)
...
def save_segmentation_results_MindBoggle(config,predictions,folder_to_save, epoch):
    ...
    if config['data']['hemi_part']=='all':
        ...
        val_ids = pd.read_csv(os.path.join(config['data']['path_to_workdir'],'MindBoggle/cortical_metrics/segmentation/half/val_L.csv')).ids
    elif config['data']['hemi_part']=='right':
        val_ids = pd.read_csv(os.path.join(config['data']['path_to_workdir'],'cortical_metrics/segmentation/half/val_R.csv')).ids
...
def save_segmentation_results_MindBoggle_test(config,predictions,folder_to_save, epoch):
    ...
    if config['data']['hemi_part']=='all':
        test_ids = pd.read_csv(os.path.join(config['data']['path_to_workdir'],'MindBoggle/cortical_metrics/segmentation/half/test.csv')).ids
    elif config['data']['hemi_part']=='left':
        test_ids = pd.read_csv(os.path.join(config['data']['path_to_workdir'],'MindBoggle/cortical_metrics/segmentation/half/test_L.csv')).ids
    elif config['data']['hemi_part']=='right':
        test_ids = pd.read_csv(os.path.join(config['data']['path_to_workdir'],'MindBoggle/cortical_metrics/segmentation/half/test_R.csv')).ids
